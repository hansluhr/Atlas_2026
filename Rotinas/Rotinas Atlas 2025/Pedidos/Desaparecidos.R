...
#Rotina para atender pedido sobre desaparecidos

#Reconstruir simdoext 2025
#SIM-DOEXT baixa o Brasil inteiro.
sim <- microdatasus::fetch_datasus(year_start = 2023, year_end = 2023,
                           vars = c("DTOBITO",  "IDADE", "SEXO", "RACACOR", "ESTCIV", "ESC", "CODMUNOCOR",
                                    "CAUSABAS","CODMUNRES","OCUP","CODESTAB"),
...
sim %>% mutate(
  ...
  instrumento = as.factor(case_when(
    ...
    causa_letra  == "Y" & causa_num == 32 | causa_letra  == "X" & causa_num == 82 ~ "Veículo") ) ) -> sim
...
#Pegar a rotina na seção sobre capitais e jovens negros. A informação sobre municípios vem do geobr. 
sim %>% mutate(
...
sim %>% tabyl(intencao, local_obito) %>% adorn_totals()
...
# Importando base homicídio oculto ----------------------------------------
...
homic_preds |>
  filter(ano == 2023) |>
  left_join(x = _, y = 
  #Mantém somente indetermindas de 2023 
  sim |>
  filter(ano ==  2023 & intencao_homic == "Indeterminado") ) |>
  #Mantém somente homicídio ocutlo
  filter(.pred_class == "homic") |> 
  #Variáveis de interesse
  select(!c(.pred_homic, .pred_n_homic, causa_letra, causa_num) ) |> 
  #Exportação
  rio::export(x = _, "desaparecidos.xlsx")
