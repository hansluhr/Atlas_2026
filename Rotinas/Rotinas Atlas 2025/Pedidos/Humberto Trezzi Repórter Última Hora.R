...
#Pedido Humberto Trezzi Repórter Última Hora

sim_doext <- fetch_datasus(year_start = 2021, year_end = 2023,
                           information_system = "SIM-DOEXT") |>
  select( c("DTOBITO",  "IDADE", "SEXO", "RACACOR", "ESTCIV", "ESC",  "CODMUNOCOR",
            "CAUSABAS","CODMUNRES","HORAOBITO","CODESTAB") ) |> clean_names()
sim_doext %>% mutate(
...
#sexo
sim_doext %>% mutate(sexo = recode(sexo,"9" = "Ignorado", "0" = "Ignorado", "1" = "Homem", "2" = "Mulher"),
                     sexo = replace_na(sexo,"Ignorado")) -> sim_doext
sim_doext %>% tabyl(ano,sexo) %>% adorn_totals(where = c("row","col")) 

#Racacor
sim_doext %>% mutate(racacor = recode(racacor, "1" = "Branca", "2"= "Preta", "3" = "Amarela", "4" = "Parda", "5" = "Indigena",
                                      "9" = "Ignorado"),
                     racacor = as.factor(coalesce(racacor,"Ignorado"))) -> sim_doext

sim_doext %>% tabyl(ano,racacor) %>% adorn_totals(where = c("row","col"))


#Estado Civíl
sim_doext %>% mutate(estciv = recode(estciv, "1" ="Solteiro", "2" = "Casado", "3"="Viúvo", "4"="Divorciado", "5" = "União Estável", "9" = "Ignorado",
                                     "0" = "Ignorado"),
                     estciv = replace_na(estciv,"Ignorado")) -> sim_doext
...
sim_doext |>
  filter(uf_resd == "Rio Grande do Sul" & intencao == "Indeterminado" & ano %in% c(2021:2023)) -> rs
rm(sim_doext)




#Importando base de ocultos
load("C:/Users/gabli/Desktop/r/SIM/bases_ocultos/homic_preds_96_23.RData")
rm(base_new,base_old,rf_fit,sim_doext,w_rfxgb)
gc()


homic_preds |>
  filter(ano %in% c(2021:2023) & uf_resd == "Rio Grande do Sul") |>
  #Variáveis de interesse
  select(!c(.pred_homic, .pred_n_homic)) |>
  
  left_join(x = _,  y = 
#Importando municípios
#Fonte: https://www.ibge.gov.br/explica/codigos-dos-municipios.php
readxl::read_excel("D:/Dropbox/Ipea/Atlas/municipios_br.xls",sheet = "munics") |>
  ...
  mutate(cod_ibge = substr(cod_ibge,1,6)) |> select(!c(uf_resd) ), by = join_by("codmunres" == "cod_ibge") ) -> base

  
left_join(base, rs ) |>
  filter(.pred_class == "homic") |>
  select(!c(intencao_homic) ) |> 
  #Exportando 
  rio::export(x = _, "oculto_rs.xlsx")
