...
#Join homicídios e população.  
left_join(x = homic, y = pop_pnadc, by = join_by("ano","cod_uf_resd" == "cod_ibge",
                                                 "def_uf_resd" == "uf") ) |>
  #O código da UF não é mais necessário
  select(!c(cod_uf_resd) ) -> base
rm(homic,pop_pnadc)
...
base |> select(ano,def_uf_resd, homicidio) |>
  ...
  #Título da Tabela
  adorn_title(placement = "top", row_name = "",
              col_name = glue::glue("Número de Homicídios de Jovens, por UF {min(year)}–{max(year)}") ) |> 
  #Exportando tabela.
  rio::export(x= _ ,"base/n_homicidio_jovem_uf_br.xlsx")
...
#Tabela formato wide do número de homicídios de homens jovens.
base |> select(ano,def_uf_resd, homicidio) |>
  pivot_wider(names_from = ano, values_from =  homicidio) %>%
  #Variações
  mutate(
  ...
  #Nota de rodapé
  add_row(
    def_uf_resd = "Fonte: MS/SVSA/CGIAE - Sistema de Informações sobre Mortalidade - SIM. Elaboração: Diest/Ipea e FBSP. Nota: O número de homicídios na UF de residência foi obtido pela soma das seguintes CIDs 10: X85-Y09 e Y35 - Y36, ou seja, óbitos causados por agressão, intervenção legal e operações de guerra. O número de óbitos foi obtido pela soma de indivíduos homens de 15 a 29 anos.") |>
  #Título da Tabela
  adorn_title(placement = "top", row_name = "",
              col_name = glue::glue("Número de homicídios de homens jovens (15 a 29 anos), por UF {min(year)}–{max(year)}") ) |> 
  #Exportando tabela.
  rio::export(x= _ ,"base/n_homicidio_homem_jovem_uf_br.xlsx")
...
base |> select(ano,def_uf_resd, tx_homic) |>
  ...
  #Título da Tabela
  adorn_title(placement = "top", row_name = "",
              col_name = glue::glue("Taxa de homicídios de homens jovens (15 a 29 anos), por UF {min(year)}–{max(year)}") ) |> 
  #Exportando tabela.
